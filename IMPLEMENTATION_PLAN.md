...
- Lọc kho sách đầy đủ từ lớp 1 đến lớp 5, theo môn học và từ khóa tìm kiếm.
...
- Kho SGK từ lớp 1 đến lớp 5 gồm 63 PDF đã được chuẩn hóa thành catalog JSON
  với 6.642 trang.
- Trang đầu của mỗi PDF được render thành ảnh bìa và hiển thị trên thẻ sách.
- Trình đọc PDF hỗ trợ hiệu ứng lật trang, điều hướng, nhập số trang, toàn màn hình
  và tải tệp gốc.
- Chat phản hồi cục bộ từ catalog và có thể phân biệt lớp, môn, tập rồi mở trực
  tiếp cuốn sách được yêu cầu.

## 2. Mapping với request.md

| Yêu cầu                                | Trạng thái FE                                                              |
| -------------------------------------- | -------------------------------------------------------------------------- |
| AI Library Assistant                   | Đã tra catalog SGK JSON và mở trực tiếp trình đọc sách                     |
...
2. Khi cần hội thoại mở rộng, thay bộ nhận diện ý định cục bộ bằng API AI nhưng
   giữ catalog JSON làm nguồn dữ liệu thư viện.
